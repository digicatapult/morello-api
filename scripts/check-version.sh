...
function check_versions_consistent () {
  ...
  local HELM_VALUES_TAG_VERSION=$(yq eval '.image.tag' ./helm/morello-api/values.yaml)
  local HELM_CHART_VERSION=$(yq eval '.version' ./helm/morello-api/Chart.yaml)
  local HELM_CHART_APP_VERSION=$(yq eval '.appVersion' ./helm/morello-api/Chart.yaml)
